      ...
      window.browserAPI.onMessageFromHome(async (message) => {
        ...
        if (message.type === 'auth-data') {
          ...
          if (message.data) {
            ...
            console.log('[抖音授权] 查找横幅元素 #auth-info-display:', infoDisplay);

            if (infoDisplay) {
              console.log('[抖音授权] 更新前的内容:', infoDisplay.textContent);

              const newContent = `🎵 ���音授权脚本已运行 | Company ID: ${messageData.company_id || '未知'} | Platform: ${messageData.platform_value || '未知'}`;
              console.log('[抖音授权] 准备更新为:', newContent);
              ...
              console.log('[抖音授权] banner 元素:', banner);
              if (banner) {
