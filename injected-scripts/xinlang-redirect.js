...
(async function () {
    ...
    const logAndSave = async (msg) => {
        ...
        await window.browserAPI?.setGlobalData('xinlang_redirect_log', redirectLog);
    };
    ...
    // 防止脚本重复注入
    // ===========================
    if (window.__XINLANG_SCRIPT_LOADED__) {
    ...
    window.__XINLANG_SCRIPT_LOADED__ = true;
    await logAndSave('✅ 标记 __XINLANG_SCRIPT_LOADED__ = true');
                ...
                try {
                    ...
                    if (message.type === 'auth-data') {
                        ...
                        if (isProcessing) {
                            ...
                            console.warn('[新浪授权] ⚠️ 正在处理中，忽略重复消息');
                            return;
                        }
                        if (hasProcessed) {
                            await logAndSave('⚠️ hasProcessed=true，跳过');
                            console.warn('[新浪授权] ⚠️ 已经处理过，忽略重复消息');
                            return;
                        }
                        ...
                        if (message.data) {
                            ...
                            try {
                                localStorage.setItem('xinlang_auth_data', authDataStr);
                                // 立即验证
                                const verify = localStorage.getItem('xinlang_auth_data');
                                if (verify) {
                            ...
                            // 方案2: globalData 备用
                            await window.browserAPI.setGlobalData('xinlang_auth_data', authDataToStore);
                            await logAndSave('💾 globalData 写入完成');

                            // 方案3: 通过 URL hash 传递（最可靠）
                            const targetUrl = 'https://mp.sina.com.cn/#/Home/Notice';
                        ...
                        // 重置处理标志（无论成功或失败）
                        isProcessing = false;
                        console.log('[新浪授权] 处理完成，isProcessing=false, hasProcessed=', hasProcessed);
                    }
                } catch (error) {
