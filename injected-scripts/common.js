...
if (typeof window.uploadVideo === "function"
    ...
    window.__COMMON_JS_FEATURES__ = {
      ...
      FIX_MULTIPLATFORM_FAILURE_REPORT_P0: {
        enabled: true,
        version: '1.2.21',
        risk: 'medium',
        files: ['baijiahao-publish.js', 'souhuhao-publish.js', 'douyin-publish.js', 'shipinhao-publish.js'],
        description: '全平台失败上报审计（8/8 平台）发现 4 个 P0 极高危缺陷，全部同源于小红书未修复前的模式。①百家号 FAIL_KEYWORDS 词表过窄，「今日发文已达上限」「内容含敏感信息」「标题重复」「操作过于频繁」都不含表中任一词 → 明确失败被上报成成功（1470-1481 与小红书缺陷④后果完全一致）；②搜狐号定时路径 confirm+alert 无人值守永久悬死（2313-2316 无特性开关、无超时兜底）；③百家号定时路径全不报（1149 提前报成功+四处失败出口无 sendStatisticsError+1212 点确定后不检测结果）；④抖音/视频号超时无失败即上报成功（douyin:2182-2188 / shipinhao:2052-2065，不查任何失败探针，「范式对齐小红书」注释对齐的是未修复前的版本）。修复：百家号词表扩充+定时路径补检测与上报、搜狐 confirm/alert 换非阻塞 toast+5 分钟超时强制收口、抖音/视频号超时收口前补查失败探针（参考小红书 FIX_XIAOHONGSHU_FAILURE_REPORT 实现）',
