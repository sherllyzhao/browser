...
(async function () {
  ...
  if (typeof window.getRandomDelayMs !== "function") {
    console.warn("[视频号授权] ⚠️ common.js 未正确加载，使用降级实现");
      ...
      window.browserAPI.onMessageFromHome(async (message) => {
        try {
          ...
          if (message.type === 'auth-data') {
            ...
            console.log('[视频号授权] 我的窗口 ID:', myWindowId, '消息目标窗口 ID:', message.windowId);

            if (!message.windowId) {
              console.error('[视频号授权] ❌ 收到的 auth-data 消息缺少 windowId，这不应该发生！已拒绝处理');
              return;
            }

            if (myWindowId !== message.windowId) {
              console.warn('[视频号授权] ⚠️ 消息不是发给我的（我是 ' + myWindowId + '，消息发给 ' + message.windowId + '），拒绝处理');
              return;
            }

            console.log('[视频号授权] ✅ windowId 匹配，安全处理消息');
