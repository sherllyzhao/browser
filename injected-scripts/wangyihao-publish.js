                                ...
                                for (const img of images) {
                                    ...
                                    try {
                                        ...
                                        if (result.errno === 0 && result.data && result.data.bos_url) {
                                            ...
                                            console.log('[网易号发布] ✅ 图片替换成功:', result.data.src.substring(0, 50));
                                        } else {
                                ...
                                for (const node of mutation.addedNodes) {
                                    if (node.nodeType === 1) {
                                        ...
                                        if (classList.includes('cheetah-message') || classList.includes('message')) {
                                            // 网易号错误提示结构：<span>图标</span><span>错误文本</span>
                                        ...
                                        for (const el of errorElements) {
                                            ...
                                            if (text && !capturedErrors.includes(text)) {
                                                capturedErrors.push(text);
                                                console.log('[网易号发布] 📨 捕获到错误信息（子元素）:', text);
                                            }
                                        }
                                    }
                                }
                                            ...
                                            const tryUploadImage = async (retryCount = 0) => {
                                                ...
                                                if (result.type === 'success') {
                                                    ...
                                                    if (submitCoverBtns.length) {
                                                        ...
                                                        console.log('[网易号发布] ✅ 已点击确定（模拟鼠标事件）');
                                                        // 等待编辑器关闭和图片保存
                                                        await delay(2000);
                                                    } else {
                                                        ...
                                                        const publishId = dataObj.video?.dyPlatform?.id;
                                                        if (publishId) {
                                                            await sendStatisticsError(publishId, '找不到提交图片按钮', '网易号发布');
                                                        }
                                                        await closeWindowWithMessage('发布失败，刷新数据', 1000);
                                                        return;
                                                    }
