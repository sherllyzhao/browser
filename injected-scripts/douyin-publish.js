        ...
        if (message.type === 'auth-data') {
          ...
          if (message.data) {
            ...
            if (infoDisplay) {
              ...
              console.log('[抖音发布] 📥 接口响应:', apiResult);
...
async function publishApi(dataObj) {
  ...
  try {
    ...
    // 先等待 .container-b0L1T6 元素存在
    // alert('Waiting for .container-b0L1T6 element...');
    await waitForElement('.container-b0L1T6', 60000); // 等待最多60秒
    // alert('✅ .container-b0L1T6 element found');

    // 再等待 .container-b0L1T6 下的 video 元素存在
    // alert('Waiting for video element inside .container-b0L1T6...');
    await waitForElement('.container-b0L1T6 video', 60000); // 等待最多60秒
    // alert('✅ Video element found');

    // 等待上传进度完成
    // alert('Waiting for upload to complete...');
    await waitForUploadComplete(120000); // 等待最多2分钟
...
async function fillFormData(dataObj) {
  ...
  try {
    ...
    // 设置封面
    const coverInput = await waitForElement('.recommendCover-vWWsHB:nth-child(1)', 5000);
    coverInput.dispatchEvent(new Event('click', { bubbles: true }));
    await new Promise(resolve => setTimeout(resolve, 1000));
    const confirmDialog = await waitForElement('.semi-modal-content.semi-modal-content-animate-show', 5000);
    const confirmBtn = await waitForElement('.semi-button.semi-button-primary', 5000, 200, confirmDialog);
