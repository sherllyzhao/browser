...
if (location.search.includes("published=true")) {
    ...
    (async function () {
        ...
        setTimeout(() => {
            ...
            } else if (typeof waitForElement === "undefined" || typeof retryOperation === "undefined" || typeof uploadImage === "undefined") {
        ...
        async function handleImageUploadAbort(dataObj, error) {
            ...
            if (publishId) {
                processedVideoIds.add(publishId);
            }
            isProcessing = false;
        ...
        async function waitForPublishEditorReady(timeoutMs = 90000) {
            const [titleInput, introInput] = await Promise.all([
                waitForElement(".d-input-wrapper .d-input input", timeoutMs),
                waitForElement(".tiptap-container .ProseMirror", timeoutMs),
            ]);

            console.log("[小红书发布] ✅ 图片上传后编辑器已就绪:", {
                hasTitleInput: !!titleInput,
                hasIntroInput: !!introInput,
            });

            return { titleInput, introInput };
        }
        ...
        function normalizeLooseCompareText(text) {
            return normalizeEditorText(text).replace(/\s+/g, "").trim();
        }

        function isEditorContentMatched(actualText, expectedText) {
            const strictActual = normalizeCompareText(actualText);
            const strictExpected = normalizeCompareText(expectedText);
            if (!strictExpected) {
                return !strictActual;
            }

            if (strictActual.includes(strictExpected)) {
                return true;
            }

            const looseActual = normalizeLooseCompareText(actualText);
            const looseExpected = normalizeLooseCompareText(expectedText);
            if (!looseExpected) {
                return !looseActual;
            }

            return looseActual.includes(looseExpected);
        }
        ...
        async function setIntroEditorContent(editor, text) {
            ...
            if (!normalized) {
                try {
                    ...
                    console.warn("[小红书发布] 清空简介失败:", error.message);
                    return false;
                }
            }
            ...
            try {
                ...
                if (view) {
                    if (typeof view.pasteHTML === "function") {
                        ...
                        const stateText = normalizeEditorText(view.state.doc.textContent || "");
                        if (isExpected(stateText) || isExpected(readCurrent())) {
                            return true;
                        }
                    }
                ...
                console.warn("[小红书发布] ProseMirror dispatch 填写简介失败:", error.message);
            }
            ...
            try {
                ...
                await delay(250);
                if (isExpected(readCurrent())) {
                    return true;
                }
            } catch (error) {
                console.warn("[小红书发布] paste 事件填写简介失败:", error.message);
            }
            ...
            try {
                ...
                await delay(200);
                if (inserted && isExpected(readCurrent())) {
                    return true;
                }
            } catch (error) {
                console.warn("[小红书发布] insertHTML 填写简介失败:", error.message);
            }
            ...
            try {
                ...
                console.warn("[小红书发布] insertText 填写简介失败:", error.message);
            }

            try {
                editor.innerHTML = buildParagraphHtml(normalized);
                editor.dispatchEvent(new InputEvent("input", {
                ...
                await delay(200);
                return isExpected(readCurrent());
            } catch (error) {
                console.warn("[小红书发布] DOM 兜底填写简介失败:", error.message);
                return false;
            }
        }

        function getFillFormHelpers() {
            const safeNormalizeEditorText = typeof normalizeEditorText === "function"
                ? normalizeEditorText
                : function (text) {
                    return (text || "")
                        .replace(/\u00A0/g, " ")
            ...
            const safeNormalizeCompareText = typeof normalizeCompareText === "function"
                ? normalizeCompareText
                : function (text) {
                    return safeNormalizeEditorText(text).replace(/\s+/g, " ").trim();
                };

            const safeHtmlToPlainText = typeof htmlToPlainText === "function"
                ? htmlToPlainText
                : function (html) {
                    if (!html) {
                        return "";
                    ...
                    return safeNormalizeEditorText(tempDiv.textContent || "");
                };

            const safeSetIntroEditorContent = typeof setIntroEditorContent === "function"
                ? setIntroEditorContent
                : async function (editor, text) {
                    const normalized = safeNormalizeEditorText(text);
                    const lines = normalized.split("\n");
                    const html = lines.length > 0
                        ? lines.map(line => {
                            const trimmedLine = line.trim();
                            return trimmedLine
                                ? `<p>${trimmedLine.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#39;")}</p>`
                                : "<p><br></p>";
                        }).join("")
                        : "<p><br></p>";
                    ...
                    await delay(250);
                    const current = safeNormalizeEditorText(editor.textContent || editor.innerText || "");
                    return safeNormalizeCompareText(current).includes(safeNormalizeCompareText(normalized));
                };

            return {
                normalizeEditorText: safeNormalizeEditorText,
                normalizeCompareText: safeNormalizeCompareText,
                htmlToPlainText: safeHtmlToPlainText,
                setIntroEditorContent: safeSetIntroEditorContent,
            };
        }
        ...
        if (!window.browserAPI) {
            ...
            try {
              ...
              console.error('[小红书发布] ❌ 保存发布页URL失败:', e);
                            ...
                            try {
                                await uploadImage(messageData);
                                await waitForPublishEditorReady();
                            } catch (error) {
                                await handleImageUploadAbort(messageData, error);
                                return;
                            }
                            try {
        ...
        await (async () => {
            ...
            try {
                ...
                if (publishData && !isProcessing) {
                    ...
                    try {
                        await uploadImage(publishData);
                        await waitForPublishEditorReady();
                    } catch (error) {
                        await handleImageUploadAbort(publishData, error);
                        return;
                    }
                    try {
    ...
    async function publishApi(dataObj) {
        ...
        try {
            ...
            await retryOperation(
                async () => {
                    const stageEle = document.querySelector("#creator-preview-image-0");
                    if (!stageEle) {
                        throw new Error(".stage 元素未找到");
                    }
                    const stageText = stageEle.getAttribute('src');
    ...
    async function fillFormData(dataObj) {
        ...
        try {
            ...
            const localHelperStatus = {
                normalizeEditorText: typeof normalizeEditorText,
                normalizeCompareText: typeof normalizeCompareText,
                htmlToPlainText: typeof htmlToPlainText,
                setIntroEditorContent: typeof setIntroEditorContent,
            };
            console.log("[小红书发布] fillFormData 依赖检查:", {
                external: externalDependencyStatus,
                local: localHelperStatus,
            });

            const missingDependencies = Object.entries(externalDependencyStatus)
                .filter(([, value]) => value !== "function")
                .map(([key]) => key);
            if (missingDependencies.length > 0) {
                throw new Error(`fillFormData 依赖缺失: ${missingDependencies.join(", ")}`);
            }

            const normalizeEditorTextSafe = typeof normalizeEditorText === "function"
                ? normalizeEditorText
                : function (text) {
                    return (text || "")
                        .replace(/\u00A0/g, " ")
            ...
            const normalizeCompareTextSafe = typeof normalizeCompareText === "function"
                ? normalizeCompareText
                : function (text) {
                    return normalizeEditorTextSafe(text).replace(/\s+/g, " ").trim();
                };

            const normalizeLooseCompareTextSafe = typeof normalizeLooseCompareText === "function"
                ? normalizeLooseCompareText
                : function (text) {
                    return normalizeEditorTextSafe(text).replace(/\s+/g, "").trim();
                };

            const isEditorContentMatchedSafe = typeof isEditorContentMatched === "function"
                ? isEditorContentMatched
                : function (actualText, expectedText) {
                    const strictActual = normalizeCompareTextSafe(actualText);
                    const strictExpected = normalizeCompareTextSafe(expectedText);
                    if (!strictExpected) {
                        return !strictActual;
                    }
                    ...
                    const looseActual = normalizeLooseCompareTextSafe(actualText);
                    const looseExpected = normalizeLooseCompareTextSafe(expectedText);
                    if (!looseExpected) {
                        return !looseActual;
                    }
            ...
            const htmlToPlainTextSafe = typeof htmlToPlainText === "function"
                ? htmlToPlainText
                : function (html) {
                    if (!html) {
                        return "";
                    ...
                    return normalizeEditorTextSafe(tempDiv.textContent || "");
                };

            const setIntroEditorContentSafe = typeof setIntroEditorContent === "function"
                ? setIntroEditorContent
                : async function (editor, text) {
                    const normalized = normalizeEditorTextSafe(text);
                    ...
                    await delay(250);
                    const current = normalizeEditorTextSafe(editor.textContent || editor.innerText || "");
                    return isEditorContentMatchedSafe(current, normalized);
                };

            const titleAndIntro = dataObj.video.video.sendlog;
            const expectedTitle = normalizeEditorTextSafe(titleAndIntro.title || "");
            let expectedIntroText = "";
            ...
            for (const selector of titleSelectors) {
                try {
                    // alert(`Trying selector: ${selector}`);
                            titleInput = await waitForElement(selector, 10000);
                    if (titleInput) {
            ...
            if (!titleInput) {
                ...
                try {
                    ...
                    if (normalizeCompareTextSafe(latestTitle) !== normalizeCompareTextSafe(expectedTitle)) {
                        throw new Error(`标题填写失败: 期望="${expectedTitle.slice(0, 80)}", 实际="${latestTitle.slice(0, 80)}"`);
                    }

                    // alert('✅ Title filled successfully');
                } catch (error) {
                    throw new Error(`标题设置失败: ${error.message}`);
                }
            }
            ...
            if (introFilled) {
                ...
                for (const selector of introSelectors) {
                    try {
                        // alert(`Trying selector: ${selector}`);
                        introInput = await waitForElement(selector, 10000);
                        if (introInput) {
                ...
                if (!introInput) {
                    throw new Error("简介输入框未找到");
                }

                const targetIntro = dataObj.video.video.content || "";
                const targetContent = normalizeEditorTextSafe(htmlToPlainTextSafe(targetIntro));

                // 检查实际内容
                const currentContent = normalizeEditorTextSafe(introInput.textContent || introInput.innerText || "");
                ...
                if (topicList.length > 0) {
                    //  删除掉所有话题
                    cleanedIntro = normalizeEditorTextSafe(removeHashTags(targetContent));
                }
                expectedIntroText = cleanedIntro;

                // 只有在标记未设置且内容不同时才填写
                if (normalizeCompareTextSafe(currentContent) !== normalizeCompareTextSafe(cleanedIntro)) {
                    const fillSuccess = await setIntroEditorContentSafe(introInput, cleanedIntro);
                    const latestContent = normalizeEditorTextSafe(introInput.textContent || introInput.innerText || "");
                    if (!fillSuccess || !isEditorContentMatchedSafe(latestContent, cleanedIntro)) {
                        throw new Error(`简介填写失败: 期望="${cleanedIntro.slice(0, 80)}", 实际="${latestContent.slice(0, 80)}"`);
                    }
            ...
            if (normalizeCompareTextSafe(latestTitle) !== normalizeCompareTextSafe(expectedTitle)) {
                throw new Error(`发布前标题校验失败: 期望="${expectedTitle.slice(0, 80)}", 实际="${latestTitle.slice(0, 80)}"`);
            }

            if (expectedIntroText) {
                const latestIntroInput = await waitForElement(".tiptap-container .ProseMirror", 5000);
                const latestIntro = normalizeEditorTextSafe(latestIntroInput?.textContent || latestIntroInput?.innerText || "");
                if (!isEditorContentMatchedSafe(latestIntro, expectedIntroText)) {
                    throw new Error(`发布前简介校验失败: 期望包含="${expectedIntroText.slice(0, 80)}", 实际="${latestIntro.slice(0, 80)}"`);
                }
            }
